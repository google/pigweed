...
TEST(I2cServiceTest, I2cWriteSingleByteOk) {
  constexpr Address kAddress = Address::SevenBit<0x01>();
  ...
  auto transactions = MakeExpectedTransactionArray({
      MockMessageTransaction(
          ...
          MakeExpectedMessageArray({
              MockWriteMessage(OkStatus(), kAddress, kExpectWrite),
          }),
          kI2cTransactionTimeout),
  });

  MockMessageInitiator i2c_initiator(transactions);
...
TEST(I2cServiceTest, I2cWriteMultiByteOk) {
  ...
  static constexpr Address kAddress = Address::SevenBit<0x01>();
...
TEST(I2cServiceTest, I2cWriteInvalidBusIndex) {
  ...
  MockMessageInitiator i2c_initiator({});
...
TEST(I2cServiceTest, I2cReadSingleByteOk) {
  ...
  auto transactions = MakeExpectedTransactionArray({
      MockMessageTransaction(
          ...
          MakeExpectedMessageArray({
              MockWriteMessage(OkStatus(), kAddress, kExpectWrite),
              MockReadMessage(OkStatus(), kAddress, kExpectRead),
          }),
          kI2cTransactionTimeout),
  });
