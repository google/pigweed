...
constexpr size_t kMaxTestMessageLength = 1024;
...
struct SocketRpcEndpoint {
  explicit SocketRpcEndpoint(SocketRpcTransport<kMaxPacketSize>& transport)
      : transport(transport),
        ...
        tx_channels({rpc::Channel::Create<1>(&rpc_egress)}),
        rx_channels({ChannelEgress{1u, local_egress}}),
...
TEST(RpcIntegrationTest, SocketTransport) {
  ...
  constexpr size_t kMessageLength = 50;
  ...
  for (int i = 0; i < 10; i++) {
    InlineString<kMaxTestMessageLength> test_message;
    test_message.append(kMessageLength, '*');
  ...
  std::array<std::byte, 1> terminator_bytes{std::byte{0x42}};
