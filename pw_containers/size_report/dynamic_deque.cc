...
int MeasureDynamicDeque(Iterator first, Iterator last, uint32_t mask) {
  static DynamicDeque<T> dynamic_deque(pw::allocator::GetLibCAllocator());
  return MeasureDeque(dynamic_deque, first, last, mask);
}

int Measure() {
  volatile uint32_t mask = bloat::kDefaultMask;
  auto& items1 = GetItems<V1>();
  int rc = MeasureDynamicDeque<V1>(items1.begin(), items1.end(), mask);

#ifdef PW_CONTAINERS_SIZE_REPORT_ALTERNATE_VALUE
  auto& items2 = GetItems<V2>();
  rc += MeasureDynamicDeque<V2>(items2.begin(), items2.end(), mask);
#endif
