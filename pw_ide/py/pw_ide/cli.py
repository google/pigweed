...
class SphinxStripper:
    ...
    def handle_lines(self, line: str = '') -> None:
        ...
        self.handled_lines.extend(
            getattr(self.handler, handler_fn,
                    lambda _, s: s)(self.tag, self.lines_to_handle))
...
class RawDescriptionSphinxStrippedHelpFormatter(
    ...
    def _handle_directive_code_block(  # pylint: disable=no-self-use
            self, tag: str, lines: List[str]) -> List[str]:
        if tag == 'bash':
            processed_lines = []
...
class _ParserAdder(Protocol):
    ...
    type annotations.
    """
    def __call__(self, subcommand_handler: Callable[..., None], *args: Any,
...
def _parser_adder(subcommand_parser) -> _ParserAdder:
    ...
    """
    def _add_parser(subcommand_handler: Callable[..., None], *args,
...
def _build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser_cpp.add_argument('-l',
                            ...
                            'language analysis')
    parser_cpp.add_argument('-g',
                            '--get',
                            dest='should_get_target',
                            action='store_true',
                            help='print the current target used for C/C++ '
                            'language analysis')
    ...
    parser_cpp.add_argument('--clangd-command-for',
    ...
    parser_python.add_argument('--venv',
