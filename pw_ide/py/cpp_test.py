...
class TestPathToExecutable(PwIdeTestCase):
    ...
    def test_valid_absolute_path_in_env_returns_same_path(self):
        ...
        result = path_to_executable(str(executable_path.absolute()),
                                    path_globs=query_drivers)
    ...
    def test_valid_relative_path_returns_same_path(self):
        ...
        query_drivers = [f'{self.temp_dir_path}/*']
        result = path_to_executable(str(executable_path),
                                    path_globs=query_drivers)
        self.assertIsNotNone(result)
...
class TestCppCompileCommand(PwIdeTestCase):
    ...
    def run_process_test_with_valid_commands(
            self, command: str, expected_target: Optional[str],
            expected_executable: Optional[str]) -> None:
        compile_command = CppCompileCommand(command=command,
                                            file='',
                                            directory='')
        ...
        self.assertEqual(processed_compile_command.target, expected_target)
        self.assertEqual(processed_compile_command.executable_name,
                         expected_executable)
    ...
    def test_process_valid_with_gn_compile_command(self) -> None:
        ...
        for case in cases:
            self.run_process_test_with_valid_commands(case['command'],
                                                      case['target'],
                                                      case['executable'])
    ...
    def test_process_unresolvable_with_gn_compile_command(self) -> None:
        ...
        processed_compile_command = cast(CppCompileCommand,
                                         compile_command.process())
        self.assertIsNotNone(processed_compile_command)
        self.assertEqual(processed_compile_command.command, command)
...
class TestCppCompilationDatabase(PwIdeTestCase):
    ...
    def test_process(self):
        ...
        compdbs = CppCompilationDatabase.load(raw_db).process(
            settings, default_path=self.temp_dir_path)
        compdbs_as_dicts = {
...
class TestCppIdeFeaturesState(PwIdeTestCase):
    ...
    def test_finds_all_compdbs(self) -> None:
        ...
        found_compdbs_str = [(get_name(target.compdb_file_path),
    ...
    def test_get_target_finds_valid_target(self) -> None:
        ...
        compdb_filename = (f'{_COMPDB_FILE_PREFIX}'
    ...
    def test_set_target_invalid_target_not_in_enabled_targets_raises(
        ...
        with self.make_temp_file(compdb_generate_file_path(
                target)), self.assertRaises(InvalidTargetException):
...
class TestAggregateCompilationDatabaseTargets(PwIdeTestCase):
    ...
    def test_gets_all_legitimate_targets(self):
        ...
        aggregated_targets = aggregate_compilation_database_targets(
            raw_db, default_path=self.temp_dir_path)
