...
class Crc32 {
 public:
  ...
  void Update(std::span<const std::byte> data) {
    state_ = ChecksumCrc32(data, state_);
  }
 ...
 private:
  ...
  static std::uint32_t ChecksumCrc32(
      std::span<const std::byte> data,
      std::uint32_t current_state = kInitialValue) {
    for (auto byte : data) {
      current_state =
          kCrc32Table[(current_state ^ static_cast<std::uint8_t>(byte)) &0xFFu] ^ (current_state >> 8);
    }
