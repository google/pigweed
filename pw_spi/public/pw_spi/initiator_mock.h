...
/// A generic mocked backend for `pw::spi::Initiator`. Represents a complete
/// parameter set for the `pw::spi::Initiator::WriteRead()`. This is
/// specifically intended for use when developing drivers for SPI devices. It's
/// structured around a set of "transactions" where each transaction contains a
/// write, read, and a status. A transaction list can then be passed to the
/// `MockInitiator`, where each consecutive call to read/write will iterate to
/// the next transaction in the list.
...
/// expectations. This MockedInitiator should be instantiated inside a Gtest
/// test frame.
