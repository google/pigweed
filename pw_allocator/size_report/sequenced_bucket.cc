...
using ::pw::containers::size_report::MeasureIntrusiveList;
...
int Measure() {
  ...
  static std::array<SequencedItem, kNumItems> items;
  if (int rc =
          MeasureIntrusiveList<SequencedItem>(items.begin(), items.end(), mask);
      rc != 0) {
    return rc;
  }

#ifndef PW_ALLOCATOR_SIZE_REPORT_BASE
  static SequencedBucket<BlockType> bucket;
