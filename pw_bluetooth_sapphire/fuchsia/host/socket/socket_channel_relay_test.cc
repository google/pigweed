...
#include "pw_bluetooth_sapphire/fuchsia/host/socket/socket_channel_relay.h"
...
class SocketChannelRelayTest : public ::testing::Test {
 public:
  ...
  [[nodiscard]] size_t StuffSocket() {
    ...
    // syscalls required.
    for (const auto spam_size_bytes : {65536,
                                       32768,
                                       16384,
                                       8192,
                                       4096,
  ...
  [[nodiscard]] bool DiscardFromSocket(size_t n_bytes_requested) {
    ...
    while (n_total_bytes_read < n_bytes_requested) {
      ...
      read_res = remote_socket_.read(0,
                                     received_data.mutable_data(),
                                     received_data.size(),
                                     &n_iter_bytes_read);
      if (read_res != ZX_OK && read_res != ZX_ERR_SHOULD_WAIT) {
        bt_log(ERROR,
               "l2cap",
               "Failure in zx_socket_read(): %s",
 ...
 private:
  ...
  // TODO(fxbug.dev/42150969): Move to FakeChannelTest, which wraps pw_async.
  async::Loop loop_;
  ...
  SocketChannelRelayLifetimeTest()
      : was_deactivation_callback_invoked_(false),
        relay_(std::make_unique<RelayT>(
            ConsumeLocalSocket(), channel()->GetWeakPtr(), [this]() {
...
TEST_F(SocketChannelRelayLifetimeTest, OversizedDatagramDeactivatesRelay) {
  ...
  const auto write_res = remote_socket()->write(0,
                                                large_message.data(),
                                                large_message.size(),
...
class SocketChannelRelayDataPathTest : public SocketChannelRelayTest {
 public:
  SocketChannelRelayDataPathTest()
      : relay_(ConsumeLocalSocket(),
               channel()->GetWeakPtr(),
               /*deactivation_cb=*/nullptr,
...
class SocketChannelRelayRxTest : public SocketChannelRelayDataPathTest {
 protected:
  DynamicByteBuffer ReadDatagramFromSocket(const size_t dgram_len) {
    ...
    const auto read_res =
        remote_socket()->read(0,
                              socket_read_buffer.mutable_data(),
                              socket_read_buffer.size(),
                              &n_bytes_read);
    if (read_res != ZX_OK) {
      bt_log(ERROR,
             "l2cap",
...
TEST_F(SocketChannelRelayRxTest,
  ...
  // The data should be copied synchronously, so the async loop should not be
  // run here.
...
TEST_F(SocketChannelRelayRxTest,
  ...
  const auto status = remote_socket()->get_info(ZX_INFO_SOCKET,
                                                &info,
                                                sizeof(info),
...
TEST_F(SocketChannelRelayTxTest, ValidSduAfterOversizedSduIsIgnored) {
...

  {
    ...
    write_res = remote_socket()->write(
        0, dropped_msg.data(), dropped_msg.size(), &n_bytes_written);
    ASSERT_EQ(ZX_OK, write_res);
...

  {
    ...
    zx_status_t write_res = ZX_ERR_INTERNAL;
    write_res = remote_socket()->write(
        0, kSentMsg.data(), kSentMsg.size(), &n_bytes_written);
    ASSERT_EQ(ZX_OK, write_res);
...
TEST_F(SocketChannelRelayTxTest,
  ...
  const auto write_res =
      remote_socket()->write(0, &data, sizeof(data), /*actual=*/nullptr);
...
TEST_F(SocketChannelRelayRxTest,
       DrainWriteQueueWhileSocketWritableWaitIsPending) {
  ...
  // Make the first 2 datagrams 1 byte each so that we can discard exactly 2
  // bytes after stuffing the socket.
  size_t n_bytes_written = 0;
  zx_status_t write_res = local_socket_unowned()->write(
      0, kSentMessage0.data(), kSentMessage0.size(), &n_bytes_written);
  ...
  // Drain enough space for kSentMessage2 and kSentMessage3, but not
  // kSentMessage1!
  ASSERT_TRUE(DiscardFromSocket(2 * kSentMessage0.size()));

  // The kSentMessage1 is too big to write to the almost-stuffed socket. The
  // packet will be queued, and the "socket writable" signal wait will start.
  channel()->Receive(kSentMessage1);
  // The second 1-byte packet that will fit into the socket is also queued.
  channel()->Receive(kSentMessage2);
  // When the third 1-byte packet is queued, kSentMessage1 will be dropped from
  // the queue, allowing kSentMessage2 and kSendMessage3 to be written to the
  // socket. The queue will be emptied.
  channel()->Receive(kSentMessage3);

  // Allow any signals to be processed.
  RunLoopUntilIdle();

  // Verify that kSentMessage2 and kSentMessage3 were actually written to the
  // socket.
  ASSERT_TRUE(DiscardFromSocket(n_junk_bytes));
  RunLoopUntilIdle();
  EXPECT_TRUE(ContainersEqual(kSentMessage2,
