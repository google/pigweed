...
fit::result<bt::att::ErrorCode> GattStatusFromFidl(GattErrorCode error_code,
                                                   bool is_read) {
  switch (error_code) {
...
void GattServerServer::PublishService(
  ...
  if (!bt::StringToUuid(service_info.type, &service_type)) {
    bt_log(WARN,
           "fidl",
           "%s: invalid service UUID %s",
           __FUNCTION__,
           service_info.type.c_str());
    auto error = fidl_helpers::NewFidlError(ErrorCode::INVALID_ARGUMENTS,
  ...
  auto read_handler = [self](bt::PeerId /*ignore*/,
                             auto svc_id,
                             auto id,
                             auto offset,
  ...
  auto write_handler = [self](bt::PeerId /*ignore*/,
                              auto svc_id,
                              auto id,
                              auto offset,
                              const auto& value,
  ...
  auto ccc_callback = [self](auto svc_id,
                             auto id,
  ...
  gatt()->RegisterService(std::move(service),
                          std::move(id_cb),
