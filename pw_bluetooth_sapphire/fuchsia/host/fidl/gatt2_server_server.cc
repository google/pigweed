...
void Gatt2ServerServer::RemoveService(InternalServiceId id) {
  ...
  services_.erase(id);
  // Unregister the service from the local GATT host. Don't remove the ID from
  // service_id_mapping_ to prevent service ID reuse.
  gatt()->UnregisterService(id.value());
}

void Gatt2ServerServer::PublishService(
    ServiceInfo info,
  ...
  if (!info.has_type()) {
    bt_log(
        WARN, "fidl", "%s: `info` missing required `type` UUID", __FUNCTION__);
  ...
  if (!info.has_characteristics()) {
    bt_log(WARN,
           "fidl",
           "%s: `info` missing required `characteristics`",
           __FUNCTION__);
  ...
  if (service_id_mapping_.count(client_svc_id) != 0) {
    bt_log(WARN,
           "fidl",
           "%s: cannot publish service with already-used `handle`",
           __FUNCTION__);
    callback(fpromise::error(PublishServiceError::INVALID_SERVICE_HANDLE));
    return;
  }
  ...
  for (const auto& fidl_chrc : info.characteristics()) {
    btg::CharacteristicPtr maybe_chrc =
        fidl_helpers::Gatt2CharacteristicFromFidl(fidl_chrc);
    if (!maybe_chrc) {
      callback(fpromise::error(PublishServiceError::INVALID_CHARACTERISTICS));
      return;
    }
  ...
  auto id_cb = [self,
                service_handle = std::move(service),
                client_svc_id,
    ...
    if (raw_id == bt::gatt::kInvalidId) {
      bt_log(INFO,
             "bt-host",
             "internal error publishing service (handle: %lu)",
             client_svc_id.value());
    ...
    auto error_handler =
        [self, client_svc_id, internal_id](zx_status_t status) {
          bt_log(INFO,
                 "bt-host",
                 "LocalService shut down, removing GATT service (id: %lu)",
                 client_svc_id.value());
    ...
    service_ptr.events().OnNotifyValue =
        [self, internal_id](ValueChangedParameters update) {
          self->OnNotifyValue(internal_id, std::move(update));
        };
    service_ptr.events().OnIndicateValue = [self, internal_id](
                                               ValueChangedParameters update,
                                               zx::eventpair confirm) {
      self->OnIndicateValue(internal_id, std::move(update), std::move(confirm));
    };
    self->services_.emplace(internal_id,
                            Service{.local_svc_ptr = std::move(service_ptr)});
  ...
  gatt()->RegisterService(
      ...
      fit::bind_member<&Gatt2ServerServer::OnClientCharacteristicConfiguration>(
          this));
}

void Gatt2ServerServer::OnReadRequest(bt::PeerId peer_id,
  ...
  auto cb = [responder = std::move(responder)](
    ...
    if (res.is_err()) {
      responder(fit::error(fidl_helpers::Gatt2ErrorCodeFromFidl(res.err())),
                bt::BufferView());
      return;
    }
    responder(fit::ok(), bt::BufferView(res.response().value));
  };
  svc_iter->second.local_svc_ptr->ReadValue(
      fbt::PeerId{peer_id.value()}, Handle{id}, offset, std::move(cb));
}

void Gatt2ServerServer::OnWriteRequest(bt::PeerId peer_id,
                                       bt::gatt::IdType service_id,
                                       btg::IdType id,
                                       uint16_t offset,
  ...
  auto cb = [responder = std::move(responder)](
    ...
    // If this was a Write Without Response request, the response callback will
    // be null.
    if (responder) {
...
void Gatt2ServerServer::OnClientCharacteristicConfiguration(
    bt::gatt::IdType service_id,
...
void Gatt2ServerServer::OnSuppressDiscovery(InternalServiceId service_id) {
  // TODO(fxbug.dev/42180948): This event is not yet supported
  bt_log(ERROR,
         "fidl",
         "%s not supported - see https://fxbug.dev/42180948",
         __FUNCTION__);
}
...
void Gatt2ServerServer::OnNotifyValue(
    InternalServiceId service_id,
  ...
  if (!update.has_peer_ids() || update.peer_ids().empty()) {
    gatt()->UpdateConnectedPeers(service_id.value(),
                                 update.handle().value,
                                 update.value(),
                                 /*indicate_cb=*/nullptr);
    return;
  ...
  for (auto peer_id : update.peer_ids()) {
    gatt()->SendUpdate(service_id.value(),
                       update.handle().value,
                       bt::PeerId(peer_id.value),
                       update.value(),
                       /*indicate_cb=*/nullptr);
  }
}

void Gatt2ServerServer::OnIndicateValue(
    InternalServiceId service_id,
    fuchsia::bluetooth::gatt2::ValueChangedParameters update,
  ...
  if (!update.has_peer_ids() || update.peer_ids().empty()) {
    auto indicate_cb =
        [confirm = std::move(confirmation)](bt::att::Result<> status) mutable {
          if (status.is_error()) {
      ...
      [pending = update.peer_ids().size(),
       confirm = std::move(confirmation)](bt::att::Result<> res) mutable {
        ...
        if (res.is_error()) {
          bt_log(INFO,
                 "fidl",
                 "failed to indicate some peers: %s",
                 bt_str(res.error_value()));
...
void Gatt2ServerServer::SubtractCredit(Service& svc) {
  ...
  if (svc.credits <= REFRESH_CREDITS_AT) {
    ...
    uint8_t credits_to_add =
        static_cast<uint8_t>(INITIAL_VALUE_CHANGED_CREDITS - svc.credits);
