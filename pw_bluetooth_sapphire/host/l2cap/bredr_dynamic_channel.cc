...
void BrEdrDynamicChannelRegistry::OnRxInfoReq(
  ...
  // TODO(fxbug.dev/933): The responses here will likely remain hardcoded
...
void BrEdrDynamicChannel::OnRxConfigReq(
  ...
  if (remote_config_accum_.has_value()) {
    ...
    // TODO(fxbug.dev/40053): if channel is being re-configured, merge with
  ...
  // accept any MTU.
  // TODO(fxbug.dev/41376): determine the upper bound of what we are actually
  // capable of sending
...
ChannelConfiguration BrEdrDynamicChannel::CheckForUnacceptableConfigReqOptions(
  ...
  // TODO(fxbug.dev/40053): reject reconfiguring MTU if mode is Enhanced
...
BrEdrDynamicChannel::CheckForUnacceptableErtmOptions(
  ...
  // NOTE(fxbug.dev/1033): MPS must be large enough to fit the largest SDU in
  // the minimum MTU case, because ERTM does not segment in the outbound
  // direction.
